...
// Add known ticker mappings
const TICKER_MAPPINGS = {
    'META': 'FB',  // Meta (Facebook) trades as FB on some APIs
...
document.getElementById('ticker-input-form').addEventListener('submit', (e) => {
    ...
    // Check for known ticker mappings
...
async function fetchStockData() {
    ...
    try {
        const stockData = await Promise.all(tickersArr.map(async (ticker) => {
            const url = `https://polygon-api-worker.abduchaib.workers.dev/?ticker=${ticker}&startDate=${dates.startDate}&endDate=${dates.endDate}`
...
async function fetchReport(data) {
    console.log('Sending data to AI worker:', data)
    const messages = [
        {
            role: "system",
        ...
        {
            role: "user",
            content: `Here is the stock data to analyze: ${JSON.stringify(data)}
    ...
    try {
        const url = `https://polygon-api-worker.abduchaib.workers.dev/api/v1/analyze`
        ...
        console.log('AI worker response:', responseData)
        ...
        console.error('Error from AI worker:', err.message)
        loadingArea.innerText = 'Unable to generate report. Please try again.'
    }
}

function renderReport(output) {
    console.log('Rendering report with output:', output)
    loadingArea.style.display = 'none'
    const outputArea = document.querySelector('.output-panel')
    const report = document.createElement('p')
    outputArea.appendChild(report)
    report.textContent = output || 'No report could be generated. Please try again.'
